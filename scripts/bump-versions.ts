...
async function main() {
    ...
    program
        .name('bump-versions')
        .description('CLI to manage package versions')
    ...
    program
        ...
        .action(async () => {
            ...
            for (const pkg of packages) {
                ...
                if (same) {
                    console.log(`${chalk.cyan(pkg.repo.repoName)} ${chalk.yellow(pkg.name)}: local ${chalk.green(pkg.currentVersion)} == published ${chalk.green(pkg.publishedVersion)}`)
                } else {
                    console.log(`${chalk.cyan(pkg.repo.repoName)} ${chalk.yellow(pkg.name)}: local ${chalk.green(pkg.currentVersion)} != published ${chalk.red(pkg.publishedVersion || 'N/A')}`)
                }
            }
        })
    ...
    program
        ...
        .action(async (type: 'major' | 'minor' | 'patch', _, options: { dryRun: boolean }) => {
            ...
            const maxVersion = maxLocalVersion!.cmp(maxPublishedVersion!) > 0 ? maxLocalVersion : maxPublishedVersion
            ...
            for (const item of plan) {
                console.log(`${chalk.blue(item.package.repo.repoName)} ${chalk.cyan(item.package.name)}: ${chalk.red(item.from)} ${chalk.gray('->')} ${chalk.green(item.to)}`)
            }
